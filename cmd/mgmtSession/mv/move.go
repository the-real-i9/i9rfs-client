...
func Run(command string, cmdArgs []string, workPath string, connStream *websocket.Conn) {
	...
	if recvData.StatusCode != 200 {
		...
		errMsgRepl := strings.NewReplacer("$source", fmt.Sprintf("'%s'", cmdArgs[0]), "$dest", fmt.Sprintf("'%s'", cmdArgs[1]), "$source_last_seg", fmt.Sprintf("'%s'", sourceSegments[len(sourceSegments)-1]))
